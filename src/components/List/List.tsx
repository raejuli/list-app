...
export const List = forwardRef<ListRef, ListProps>(({initialItems, onItemAdded, onItemDeleted}, ref) =>
{
    ...
    const handleAddItem = async () => {
        ...
        setIsAdding(true);
        try {
            const response = await fetch(`https://listapi.${window.location.hostname.split('.')[1]}/api/items`, {
                method: 'POST',
                headers: {
    ...
    const handleDeleteItem = async (id: number) => {
        setDeletingId(id);
        try {
            const response = await fetch(`https://listapi.${window.location.hostname.split('.')[1]}/api/items/${id}`, {
                method: 'DELETE',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
